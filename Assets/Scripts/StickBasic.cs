...
public enum LimbSelect
{
    right = 1,
    left = 0
}

public class StickBasic : MonoBehaviour
{
    ...
    private Muscle[] upper_leg = new Muscle[2];
    private Muscle[] lower_leg = new Muscle[2];
    ...
    void Start()
    {
        ...
        resetJumpVal();
        upper_leg[(int)LimbSelect.left] = muscles[2];
        upper_leg[(int)LimbSelect.right] = muscles[3];

        lower_leg[(int)LimbSelect.left] = muscles[4];
        lower_leg[(int)LimbSelect.right] = muscles[5];
    ...
    void FixedUpdate()
    {
        ...
        walkDirection = (int)Input.GetAxisRaw("Horizontal");
    ...
    {
        return syntheticHandObj.GetComponent<GunScript>().flippedBool;
    }
}
